...
double	cylinder(t_ray *ray, t_object *obj)
{
	...
	, obj->vec_dir)), vec_sub(delta_p,
	vec_kscale(vec_dot(delta_p, obj->vec_dir), obj->vec_dir)));
	c = vec_dot(vec_sub(delta_p,
	vec_kscale(vec_dot(delta_p, obj->vec_dir), obj->vec_dir))
	, vec_sub(delta_p, vec_kscale(vec_dot(delta_p, obj->vec_dir)
	, obj->vec_dir)))
...
double	cone(t_ray *ray, t_object *obj)
{
	...
	a = pow(cos(obj->alpha), 2)
	* vec_dot(vec_sub(ray->dir, vec_kscale(v_va, obj->vec_dir))
	, vec_sub(ray->dir, vec_kscale(v_va, obj->vec_dir)))
	- pow(sin(obj->alpha), 2) * v_va * v_va;
	b = 2 * pow(cos(obj->alpha), 2)
	* vec_dot(vec_sub(ray->dir, vec_kscale(v_va, obj->vec_dir))
	, vec_sub(delta_p, vec_kscale(vec_dot(delta_p
	, obj->vec_dir), obj->vec_dir)))
	- 2 * pow(sin(obj->alpha), 2) * v_va * vec_dot(delta_p, obj->vec_dir);
	c = pow(cos(obj->alpha), 2)
	* vec_dot(vec_sub(delta_p, vec_kscale(vec_dot(delta_p, obj->vec_dir),
	obj->vec_dir))
	, vec_sub(delta_p, vec_kscale(vec_dot(delta_p
	, obj->vec_dir), obj->vec_dir)))
